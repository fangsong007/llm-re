...
# -------------------------- 绘制 AUC-PR 曲线 --------------------------
def plot_pr_curve(true_labels, pred_labels, relation_types):
    # 将真实标签和预测标签转换为 One-hot 编码
    y_true = label_binarize(true_labels, classes=relation_types)
    y_pred = label_binarize(pred_labels, classes=relation_types)
    ...
    # 对于每个类别计算 Precision-Recall 曲线和 Average Precision 值
    for i, rel in enumerate(relation_types):
        precision, recall, _ = precision_recall_curve(y_true[:, i], y_pred[:, i])
        avg_precision = average_precision_score(y_true[:, i], y_pred[:, i])
        plt.plot(recall, precision, label=f"{rel} (AP = {avg_precision:.3f})")

    plt.xlabel("召回率 (Recall)")
    plt.ylabel("精确率 (Precision)")
    plt.title("多分类 AUC-PR 曲线")
...
# 绘制多分类 AUC-PR 曲线
plot_pr_curve(true_labels, pred_labels, labels)

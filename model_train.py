...
if __name__ == "__main__":
    ...
    model = BertForSequenceClassification.from_pretrained(
        model_dir,
        num_labels=len(relation_type_mapping)
    )
    ...
    train_dataset = EventRelationDataset(data_file, tokenizer, max_length, device)
    model, epoch_losses = train(model, train_dataset, device, epochs=epochs, batch_size=2)
    ...
    with open('training_info.txt', 'w') as f:
        ...
        for i, loss in enumerate(epoch_losses):
            f.write(f"Epoch {i + 1} Average Loss: {loss:.4f}\n")